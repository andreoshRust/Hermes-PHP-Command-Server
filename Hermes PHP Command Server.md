...
Система создана для того, чтобы **AI-агент (Hermes) мог выполнять команды на компьютере с Windows** через веб-интерфейс, без необходимости в сложной настройке WSL, Docker или дополнительных агентов. Права доступа агента определяются его учетной записью в Windows (настраиваются из SetupHermesUser.bat)
